...
class Q3toQL:
        ...
        for ev in self.result["events"]:
            ...
            try:
                if isinstance(ev["DATA"]["KILLER"]["STEAM_ID"], PlayerId):
                    ev["DATA"]["KILLER"]["STEAM_ID"] = ev["DATA"]["KILLER"][
                        "STEAM_ID"
                    ].steam_id  # noqa
            except KeyError:
                pass
            try:
                if isinstance(ev["DATA"]["VICTIM"]["STEAM_ID"], PlayerId):
                    ev["DATA"]["VICTIM"]["STEAM_ID"] = ev["DATA"]["VICTIM"][
                        "STEAM_ID"
                    ].steam_id  # noqa
            except KeyError:
                pass
    ...
    def on_server_time(self, ts, name, args):
        ...
        start_date = datetime.strptime(data.split()[0].strip(), "%Y%m%d%H%M%S")

        self.result["start_date"] = start_date

        logger.debug(
            "Game started at {}, server timestamp {}".format(start_date, ts)
        )
    ...
    def on_client_info_changed(self, ts, name, args):
        ...
        try:
            ...
            if user_info["name"] != player["name"]:
                new_id = self.create_steam_id(user_info["name"])
                logger.info(
    ...
    def on_kill(self, ts, name, args):
        ...
        if not weapon_name.startswith("MOD_"):
            raise ParsingError(
                "Invalid weapon '{}' at event '{}'".format(
                    weapon_name, (ts, name, args)
                )
            )
        ...
        ev_death = {"TYPE": "PLAYER_DEATH", "DATA": data}
        ev_kill = {"TYPE": "PLAYER_KILL", "DATA": data}
    ...
    def on_exit(self, ts, name, args):
        ...
        if self.result["start_date"]:
            self.result["finish_date"] = self.result["start_date"] + timedelta(
                seconds=time
            )
        ...
        self.match_report_event = {
            ...
            "DATA": {
                ...
                "CAPTURE_LIMIT": int(
                    self.result["server_info"]["capturelimit"]
                ),  # noqa
